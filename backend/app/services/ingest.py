...
from .gcs_sync import backup_index_to_gcs, backup_postgres_to_gcs
...
def run_ingest(db: Session, queries: list[str], locations: list[str], max_results_per_query: int = 25) -> dict:
    ...
    # Vectors go to FAISS, keyed by the same row_id used in Postgres
    ...
    # No-op locally. On the VM, pushes the freshly updated index files
    # AND a Postgres dump to GCS, so a replaced/restarted VM can restore
    # instead of starting from empty.
    import os
    index_backup_result = backup_index_to_gcs()
    db_url = os.environ.get("DATABASE_URL")
    db_backup_result = backup_postgres_to_gcs(db_url) if db_url else {"skipped": "no DATABASE_URL"}
    ...
    return {
        ...
        "new": inserted,
        "per_source": per_source_counts,
        "gcs_index_backup": index_backup_result,
        "gcs_db_backup": db_backup_result,
    }
